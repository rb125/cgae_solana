...
function getBackendWsUrl(): string | null {
  ...
  const explicit = process.env.NEXT_PUBLIC_WS_BASE || process.env.NEXT_PUBLIC_WS_URL;
  if (explicit) {
    return explicit;
  }

  // 2. Derive from API if it's an absolute URL
  ...
  // Note: On Vercel with relative paths (API=""), we must provide NEXT_PUBLIC_WS_BASE
  // because Next.js rewrites don't support WebSocket proxying out of the box.
  return null;
}
